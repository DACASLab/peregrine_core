"""@file
@brief Example 10 SE3 launch: PX4 SITL + minimal SE(3) manager-chain demo.

This launch starts PX4 SITL itself, then brings up the minimal single-UAV data
plane (MicroXRCE + hardware abstraction + TF) together with the manager chain
needed for SE(3) control. It intentionally does not use the safety-monitor
bringup path so SE(3) tuning stays focused on estimation, control, trajectory,
and uav_manager behavior.

Usage:
  ros2 launch hardware_abstraction_example example10_se3_circle_figure8_demo.launch.py
  ros2 launch hardware_abstraction_example example10_se3_circle_figure8_demo.launch.py mission_type:=circle
"""
...
def generate_launch_description() -> LaunchDescription:
    """@brief Starts PX4 SITL, minimal manager chain, and the SE3 demo mission."""
    ...
    start_circle_evaluator = LaunchConfiguration("start_circle_evaluator")
    ...
    default_mission_yaml = PathJoinSubstitution(
        [FindPackageShare("hardware_abstraction_example"), "config", "circle_figure8_se3_mission.yaml"]
    )
    ...
    demo_node = Node(
        ...
        namespace=uav_namespace,
        output="screen",
        parameters=[mission_config_file, {"mission_type": mission_type}],
    )

    evaluator_node = Node(
        package="hardware_abstraction_example",
        executable="circle_tracking_evaluator.py",
        namespace=uav_namespace,
        output="screen",
        parameters=[{"output_path": evaluator_output_path}],
        condition=IfCondition(start_circle_evaluator),
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "start_circle_evaluator",
                default_value="false",
                description="Start the circle-tracking evaluator node and write metrics to evaluator_output_path.",
            ),
            DeclareLaunchArgument(
                "evaluator_output_path",
                default_value="/tmp/circle_eval.json",
                description="Output path for circle-tracking metrics.",
            ),
            DeclareLaunchArgument(
                "mission_config_file",
                default_value=default_mission_yaml,
                description="Mission config YAML to load for the demo node.",
            ),
            DeclareLaunchArgument(
