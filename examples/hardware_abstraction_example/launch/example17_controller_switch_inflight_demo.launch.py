"""@file
@brief Example 17 launch: repeated in-flight controller switching.

Starts the same stack as example16, but runs a multi-switch in-flight sequence:
  takeoff (passthrough) -> switch to SE3 -> circle -> switch to passthrough ->
  figure8 -> optional switch to SE3 -> circle -> land

This stresses lifecycle switching behavior during active flight.
"""
...
def generate_launch_description() -> LaunchDescription:
    """@brief Starts composed manager stack plus in-flight controller-switch demo."""
    ...
    mission_yaml = PathJoinSubstitution(
        [
            FindPackageShare("hardware_abstraction_example"),
            "config",
            "controller_switch_inflight_mission.yaml",
        ]
    )
    ...
    demo_node = Node(
        ...
        executable="controller_switch_inflight_demo.py",
        namespace=uav_namespace,
        output="screen",
