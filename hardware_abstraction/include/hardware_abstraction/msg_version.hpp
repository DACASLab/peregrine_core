...
{
  // if constexpr: only the selected branch is instantiated, so T::MESSAGE_VERSION
  // is safe to reference even when the HasMessageVersion<T> trait resolves to false.
